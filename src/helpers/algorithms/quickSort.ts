...
const partition = (dupBlocks: number[], l: number, r: number): number => {
    const pivot = l
    let j = l
...
const quickSortHelper = (dupBlocks: number[], l: number, r: number) => {
    if (l >= r) {
...
const quickSort = (
    blocks: number[]
): Array<[number | null, number | null, number[] | null, number | null]> => {
